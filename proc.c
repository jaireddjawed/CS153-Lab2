...
exit(int status)
{
  ...
  uint time;
  time = getCurrentTime();
  curproc->endTime = time - curproc->startTime;
  cprintf("The end time for this process is (%d).\n", curproc->endTime);
...
void setpriority(int priority) {
  int numPriority = priority;

  if (numPriority <= 1) {
    numPriority = 1;
  } else if (numPriority >= 17) {
    numPriority = 16;
  }
...
inline uint getEndTime(void) {
  struct proc* curproc = myproc();
  uint time = getCurrentTime();

  curproc->endTime = time - curproc->startTime;
  cprintf("End time for this process is (%d).\n", time);
